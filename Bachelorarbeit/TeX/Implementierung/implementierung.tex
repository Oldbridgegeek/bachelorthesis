...
\For {k=1 < N }
	\For {i= 1 < N }
			\State $z_i := w^{(1)}_m b_{11} + \dots + w^{(N)}_m b_{1N}$
		\EndFor
\EndFor
...
Wir haben in \ref{eq:matrix} eine Matrix-Vektor Multiplikation von einer Matrix der Größe $N^3 \times N^3$. Dementsprechend hätten wir $N^{6}$ Multiplikationen und $N^6$ Additionen. Die Komplexitätsklasse entspricht dann $O(2N^6)$. Die Komplexität des vorgeschlagenen Algorithmuses reduziert die Operationen auf $3N^{4}$ Multiplikationen und genau so viele Additionen. Somit haben wir die Komplexitätsklasse $O(6N^4)$. Das ist eine beträchltiche Reduktion, vor allem für großes $N$.
\newline
